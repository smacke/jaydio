...
public final class OpenFlags {
    ...
    private static int getOpenFlags(final String s) {
        try {
            final Process pb = new ProcessBuilder("/bin/sh", "-c",
                    "printf \"%d\" $(printf '#include<fcntl.h>\\n" + s + "' | gcc -D_GNU_SOURCE -E - | tail -n1)")
            ...
            if (pb.waitFor() != 0 || collect.size() != 1)
                throw new RuntimeException("Cannot get C macro " + s + " " + collect);
            return Integer.parseInt(collect.get(0));
        } catch (IOException | InterruptedException e) {
            throw new RuntimeException("Cannot get C macro " + s, e);
        }
    }
}
